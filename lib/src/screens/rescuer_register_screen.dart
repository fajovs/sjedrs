...
class RescuerRegisterScreen extends StatefulWidget {
  ...
  @override
  _RescuerRegisterScreenState createState() => _RescuerRegisterScreenState();
}
  ...
  void _register() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      if (password != confirmPassword) {
        setState(() {
          _registrationError = 'Passwords do not match';
        });
        return;
      }
      ...
      try {
        final result = await _authService.signUpRescuer(
          email: email,
          password: password,
          name: name,
          phone: phone,
        ...
        if (result == 'Rescuer signed up successfully') {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      TextButton(
                        ...
                        child: const Text('Login as Rescuer', style: TextStyle(color: Colors.blue)),
                      ),
                      const SizedBox(height: 16),
                      TextButton(
                        onPressed: () {
                        ...
                        child: const Text('Back to User Login', style: TextStyle(color: Colors.blue)),
                      ),
                      const SizedBox(height: 16),
                    ],
                  ),
